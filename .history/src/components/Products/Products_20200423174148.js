...
export default class Products extends Component {
  constructor(props) {
    super(props);
    this.state = { product: null, queryString: null, nameQueryString: null };
  }
  ...
  render() {
    let { product, queryString, nameQueryString } = this.state;

    return (
      <HrFrComp>
        {(queryString === undefined || queryString === "") &&
        (nameQueryString === undefined || nameQueryString === "") ? (
          <All type="product" title="All Products" />
        ) : product === null ? (
          <Loading />
        ) : product.length === 0 ? (
  ...
  helperFunc = async () => {
    ...
    this.setState({ product, queryString, nameQueryString });
  };
