...
import { faEye, faLock, faUser, faEnvelope, faDollarSign } from '@fortawesome/free-solid-svg-icons';
...
class BenshadaForm extends React.Component {
  ...
  onSubmit = (formValues) => {
    ...
    this.props
      ...
      .then((response) => {
        response = typeof response === 'string' ? response : response.payload.message || 'Successful';
  ...
  renderFormControl(input, name, label, varClass, type, options, className, meta) {
  ...
  renderFormField = ({ input, label, meta, varClass, type, options, icon, row }) => {
    const { name } = input;
      ...
      >
        <div className="w-100">
          ...
          <div className="input-group">
            ...
            {this.renderFormControl(input, name, label, varClass, type, options, className, meta)}
  ...
  renderFields(fields) {
    return fields.map((field, key) => {
      let { desc, placeholder, varClass, type, options, icon, row, value } = field;
  ...
  renderButtons(buttons) {
    return buttons.map((button, key) => {
      ...
      return (
        <button key={key} className={`${className} mt-3 w-100 btn text-uppercase border border-0`}>
          {value}
        </button>
      );
    });
